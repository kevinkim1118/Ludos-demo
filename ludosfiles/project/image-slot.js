...
(() => {
  ...
  function getSlot(id) {
    const v = slots[id];
    if (!v) {
      // Static fallback: images extracted to real files under images/ and
      // listed in image-manifest.js. This is what makes the design render on
      // a plain static host with no editing sidecar present.
      const st = (window.IMAGE_SLOT_STATIC || {})[id];
      if (st && st.src) return { u: st.src, s: st.s || 1, x: st.x || 0, y: st.y || 0, _static: true };
      return null;
    }
    const n = typeof v === 'string' ? { u: v, s: 1, x: 0, y: 0 } : v;
    if (!n.u) {
      const st = (window.IMAGE_SLOT_STATIC || {})[id];
      if (st && st.src) return Object.assign({}, n, { u: st.src, _static: true });
    }
    return n;
  }
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '  <div class="empty" part="empty">' + icon.replace('<svg ', '<svg part="icon" ') +
        '    <div class="cap"></div>' +
        ...
        '</div>' +
        '<div class="ctl" part="ctl"><button data-act="replace" title="Replace image">Replace</button>' +
    ...
    _render() {
      ...
      if (stored && stored.u && !stored._static && !/^data:image\//i.test(stored.u)) stored = null;
      const srcAttr = this.getAttribute('src') || '';
      // Static entries stay out of _userUrl so a reframe never writes a file
      // path back into the sidecar as though it were a dropped data-URL.
      this._userUrl = (stored && !stored._static && stored.u) || null;
      const url = this._userUrl || (stored && stored._static && stored.u) || srcAttr;
