# Game Discovery Platform — Session Handoff (v4)

*A "Letterboxd for gamers" — game discovery + backlog management. This supersedes the previous handoff (v3). It captures what this session decided, built, and left open so a new chat can pick up without re-litigating settled work.*
...
> **Update (latest sessions): hi-fi is underway — four screens now built, and the flagship verdict module is done.** The **Discover / Home**, **Library / backlog**, **Game detail**, and **Head-to-head** mobile surfaces are all live, interactive hi-fi Design Components (Discover Home.dc.html, Library.dc.html, Game Detail.dc.html, Head-to-Head.dc.html) in the Crimson noir dark theme — the first four screens out of grayscale. See their dedicated sections below. Discover and Library went ahead of the verdict module; the **Game detail build delivered the verdict module in hi-fi as part of the page**, so the long-standing "next flagship" is now shipped inside a real screen. The Library build also resolved the previously-open custom-list-detail question (its Edit List screen supports title/description edits, a ranked/unordered toggle, and drag-to-reorder). **This session's build (Head-to-head) resolves the outcome-to-Library handoff question** (see its section below).
...
## Second hi-fi screen — Library / backlog, mobile (this session)

The project's **second screen taken to full hi-fi**, again a **live, interactive Design Component**: `Library.dc.html` (mobile, Crimson noir dark theme, Familjen Grotesk + Newsreader). It realizes the lo-fi Library / backlog wireframe as a working, tappable surface. A flattened, self-contained export also exists on disk (`Library (standalone).dc.html` / `Library (standalone).html`).

**What's built & interactive:**
- **Two top-level views behind a moving tab underline: Games and Lists.** Games is the status-segmented backlog (Backlog / Playing / Finished / All with live counts, per the locked want→playing→finished model); Lists is the custom-list surface — finally giving the detail page's "+ Add to a list" a real home.
- **Collection cards** with a **three-up cover preview + "+N" overflow badge**; the three preview tiles are `image-slot` drop targets (user drops real cover art; drops persist and clicks inside don't trigger the card's open).
- **List detail view** slides in from the right (and back out on close) — a scoped browser for one list, with the list's title + **per-list description** (each list keeps its own description independently; no longer a shared instance).
- **Two list layouts.** *Favorite Games* is a **ranked** list (numbered vertical rows, eight literal editable entries — title / platform / description all directly in-place editable). *Cozy night-ins* was a card grid, then **converted to the same vertical-row layout** for consistency. Each row's cover is a rounded `image-slot`; a fixed-height dashed **"Add a game"** button (matched to row height, always sorted last) closes each list.
- **Two bottom sheets with full reverse animation** — a **"How much time have you got?"** time picker and a **"Your lists"** sheet. Both slide up + fade in on open and **slide down + fade out on close** (the reverse of entry), matching the Discover sheet behavior.
- **Edit List screen** — the session's headline addition. The pencil icon in a list detail raises a **full-screen sheet from the bottom** (Cancel / Save header) to edit: the list **title** and **description** (text fields); a **"Ranked list" toggle** that shows/hides the position numbers across both list layouts; and a **drag-to-reorder game list** (HTML5 drag-and-drop; rows reorder live, position numbers renumber, and the order + ordered-flag + title/description commit back to that specific list on Save). An **"Add games"** button sits below the reorderable list. Reordering is available whether or not the list is marked ordered; numbers appear only when it is.
- Covers throughout are `image-slot` placeholders (persistent user drops), consistent with Discover.

**Fidelity / consistency held:** dark Crimson noir tokens throughout; **no numeric/star scores** (sentiment/utility only); the **crimson restraint rule** respected (crimson reserved for primary actions like Save and the add affordances). The ranked/unordered toggle makes the "cards reweight by status / lists can be ordered or not" lo-fi intent concrete and interactive.

**Resolves a lo-fi loose end:** the **custom-list detail view** — previously an open question ("tapping a custom list presumably opens the same browser scoped to that list, but that screen isn't drawn") — is now built *and* editable, including reorder/ranking.

---

## Third hi-fi screen — Game detail, mobile (this session) — ships the verdict module

The project's **third screen taken to full hi-fi**, and the most consequential: `Game Detail.dc.html` (mobile, Crimson noir dark theme, Familjen Grotesk + Newsreader), framed in the same iPhone-style device shell as the other two. Running example is **Elden Ring**, defaulting to the **untracked / deciding** condition (a `initialStatus` prop can seed want/playing/finished). **This build delivers the verdict module in hi-fi — the differentiator the foundations handoff named as the pending flagship — inside a complete, tappable page rather than in isolation.**

**What's built & interactive (top to bottom, following the lo-fi module stack):**
- **Immersive hero** — full-bleed key-art `image-slot` with a gradient scrim, floating glass **back** (routes to `Discover Home.dc.html`) and **share** buttons over a floating status bar; developer/year/genre line, title, platform pills, and a horizontally-scrolling **tag strip** (Open world · High challenge · Solo · …).
- **The verdict module (the flagship)** — the three trust tiers in locked ladder order, each its own card: **Friends** (avatar stack + "3 friends finished — all really liked it"), **Completionists / your type** (a four-bucket **sentiment distribution bar** + key, plus a neutral "Strong match" relate callout), and **Global** (its own distribution + the divergence insight, "divisive on difficulty"). Archetype badge ("The Completionist") sits in the header. Honors the **four-point sentiment** grammar and the **crimson restraint rule** (crimson only on the positive sentiment pole + primary actions; relate/alignment cues stay neutral).
- **What it'll cost you** — time-to-beat (main / completionist) + a store price list with Game Pass "Included" highlighted.
- **Friend activity** — per-friend rows (avatar · name · status pill · Newsreader-italic snippet · sentiment pill).
- **Reviews — tabbed by trust tier**: **Friends / People like you / All**, each with its own review set, an animated swap on tab change, and a tier-appropriate "see all" footer.
- **Discovery carousel** — "Played by Completionists," at-a-glance cover cards with match-strength markers (covers are `image-slot`s; **rounded corners fixed on the card border to match the image radius**).
- **Minimal contribution prompt** — a dashed "Finished it? Rate it" nudge (non-blocking, per the locked Option-B decision).
- **Sticky bottom action bar → status sheet → add-to-list sheet.** The bar's primary button raises the **status sheet** (Add to backlog / Mark as playing / Mark as finished + Add to a list), with the current status pre-selected and full **reverse** open/close animation + scrim (matching Discover/Library). Setting a status flips the bar to show it live and fires a toast. Both the sheet's **"Add to a list"** action *and* the bar's dedicated list-icon button open a second **"Add to a list" sheet** — a tall slide-up modeled on Apple Music's add-to-playlist: a **search field that live-filters the user's lists**, a "New list" action, and list rows (fillable cover art + name + count) with a **tap-to-toggle add control** (crimson ✓ / "Already in this list" when added).

**Cross-screen wiring (this session):** the **Discover / Home spotlight** was switched to **Elden Ring** and its **"View"** button now navigates to `Game Detail.dc.html`, so Discover → detail is a working path.

**Tweakable props:** `gameTitle`, `initialStatus` (none / want / playing / finished).

**Fidelity / consistency held:** dark Crimson noir tokens throughout; **no numeric/star scores anywhere** (sentiment pills + four-bucket distributions only); trust ladder order (friends → taste → global) load-bearing in both the verdict and the review tabs; crimson restraint respected.

**Resolves open threads:** (1) the **verdict module hi-fi** — the named next flagship — is now built; (2) the **mobile sheet interaction details** (slide-up, scrim, pre-selected current status, reverse animation) are now realized, not just described; (3) the **"+ Add to a list" destination** is now a real, searchable screen.

---

## Fourth hi-fi screen — Head-to-head, mobile (this session)

The project's **fourth screen taken to full hi-fi**: Head-to-Head.dc.html (mobile, Crimson noir dark theme, Familjen Grotesk + Newsreader), framed in the same iPhone-style device shell. It realizes the lo-fi head-to-head wireframe set as a working, tappable **winner-stays state machine** — not a static mock. Entered from the Library's "What to play next" hero (the "Still can't decide?" bar now routes here for real).

**What's built & interactive, matching the four lo-fi frames:**
- **Intent** — the four mood options (Something fast & fun · Chip away at a big game · Get lost in a story · Something chill) as selectable rows with an animated check; each scopes a real game pool. **"Chip away at a big game" was relabeled "Long Haul"** per this session's copy pass. "Start comparing" begins the duel on the selected pool.
- **Duel** — the Beli-style side-by-side with the **OR** badge; tapping a card *is* the pick (highlight, scale-up, and the loser dims before the next challenger swaps in). A real **champion-vs-challenger queue** drives it: the winner holds its seat, the next pool game loads opposite it. **Undo** (steps back through pick history, disabled/greyed until there's a pick to undo) and **Skip this matchup** (free, doesn't count toward streak/cap) are both wired, not decorative. The **~5-tick progress row** and the **tappable intent chip** (jumps back to Intent to change mood) are live.
- **Stop rule implemented exactly as specified:** auto-resolves at the first of a **3-in-a-row streak**, a **5-compare cap**, or an exhausted queue — each produces a distinct one-line reason on the outcome screen (no reason copy for the exhausted case, per this session's trim).
- **Outcome** — winning cover (now sized to the **2:3 / 600×900 Steam vertical-cover ratio** across every cover slot in the flow, this session's sizing pass) + name + reason chip. **No launch CTA** (per the locked decision) — the primary action reads **"Mark [game] as playing"** (copy fixed this session from an earlier "Start playing," which wrongly implied the app launches the game). Secondary: **Keep comparing** (re-rolls a fresh duel from the same intent's remaining pool) and **Back to library**.
- **Cold-start** — the in-flow screen (one real card + a dashed "Add a game" slot), shown live when an intent's pool has fewer than two games (e.g. "Something chill"); offers **Change your mood** / **Browse Discover**.
- **Covers are image-slot placeholders** at the corrected 2:3 ratio; user-dropped art persists per game.

**This session resolves the previously-open "outcome → Library handoff" question:** the winner is **not** pinned or written back as the Library hero's confident pick. Instead, marking the winner as playing writes a one-shot flag that Library reads on load and surfaces as a **toast** ("Marked [game] as playing") — a lightweight confirmation, not a data handoff. Back/return from any head-to-head screen routes to Library.dc.html.

**Small cross-screen fixes bundled in this session's polish pass:**
- **Library's "What to play next" re-roll bug fixed** — "Another" was swapping the recommended game's text but reusing one shared cover slot; each recommended game now gets its own keyed image-slot (cv-pick-&lt;gamekey&gt;), so the cover swaps correctly too.
- **Friends feed chevron rule tightened.** The "›" affordance (implying a tap-through to the game's detail page) now shows **only when the card's headlined action carries a written blurb** (a left review, or a finish with a snippet) — ratings-only, playing-started, and list-add cards no longer show it, since they have nothing to read on the other side of the tap.
- **Genre label fix:** Outer Wilds recategorized **Mystery → Exploration** (better fits the game).

**Tweakable props:** startScreen (intent / duel / outcome / coldstart — jump straight to any state) and initialIntent (fast / big / story / chill).

**Fidelity / consistency held:** dark Crimson noir tokens throughout; **no numeric/star scores anywhere** (cards carry only cover · name · genre · time-to-finish, per the locked sparse-card decision); crimson reserved for the primary pick states and CTAs; the champion-vs-challenger queue and stop rule match the wireframe's locked mechanic exactly, not just its look.
...
- **Sheet interaction details (mobile) — now built.** The slide-up animation, the scrim, pre-selecting the current status, and full reverse open/close are realized in `Game Detail.dc.html` (status sheet + add-to-list sheet). No longer open.
...
- **Head-to-head "compare two" flow — now built in hi-fi** (Head-to-Head.dc.html; see its dedicated section above). The **outcome → Library handoff is resolved**: no pinning/hero hand-off, just a one-shot toast on return to Library. Still open: whether the Library hero's own "How much time?" duration chip should feed the intent screen or stay independent (deliberately left untouched — it's this open call).
...
- **Custom list detail view — now built (this session).** Resolved in the Library hi-fi build: tapping a list opens a right-sliding scoped browser, with a full-screen **Edit List** sheet (title/description edit, ranked toggle, drag-to-reorder + add games). No longer open.
...
*Current direction: design-system foundations are locked (Crimson noir, both themes, type pairing); three hi-fi mobile screens are shipped including the flagship verdict module — hi-fi work continues from the foundations.*
...
3. **First four hi-fi screens shipped: Discover / Home + Library / backlog + Game detail + Head-to-head (mobile), interactive.** `Discover Home.dc.html`, `Library.dc.html`, `Game Detail.dc.html`, and `Head-to-Head.dc.html` — Game Detail **ships the verdict module in hi-fi** (three trust tiers, four-bucket distributions, divergence insight) plus the full status-sheet → add-to-list-sheet interaction chain, and wires Discover's spotlight "View" → the Elden Ring detail page; Head-to-Head ships the full champion-vs-challenger state machine (undo/skip/stop-rule) and resolves the outcome→Library handoff as a toast. Together they prove the palette + type + component grammar in real builds. Still to do: the **desktop** counterparts in hi-fi, and possibly the light theme.
4. **Verdict module — now shipped (in the Game detail build).** The differentiator is out of grayscale and interactive. Remaining verdict work is variant states (finished/reviewed re-anchor, cold-start inversion) rather than the core module.
5. **Finish the remaining loose ends** (Library empty/cold-start, friends-feed multi-action + empty cold-start, onboarding quiz skip path). Custom-list detail and the head-to-head **outcome → Library handoff** are now both done (built in hi-fi). These can now be drawn/built directly in the new palette rather than grayscale.
...
*Last updated: fourth hi-fi screen session — built **Head-to-Head.dc.html**, the Head-to-head mobile surface, as a live interactive Design Component in Crimson noir. Realizes all four lo-fi frames (Intent / Duel / Outcome / Cold-start) as a real **champion-vs-challenger, winner-stays state machine** with working Undo/Skip and the locked stop rule (3-streak / 5-cap / exhausted-queue), each with its own outcome reason. Covers across the whole flow resized to the **2:3 Steam vertical-cover ratio**. Resolves the open **outcome → Library handoff** question (one-shot toast on return, no pinning/hero hand-off). Copy fixes: "Chip away at a big game" → **"Long Haul"**; outcome CTA "Start playing" → **"Mark [game] as playing"** (the app can't launch games); Outer Wilds genre "Mystery" → **"Exploration"**; removed a stray double space. Bundled fixes: Library's "What to play next" **re-roll now swaps the cover, not just the text** (per-game keyed image-slots); Friends feed's tap-through **chevron now shows only on cards with a written blurb** (review or snippet), not on ratings/playing/list-add cards. Tweakable props: startScreen, initialIntent.*

*Prior: third hi-fi screen — built **Game Detail.dc.html**, the Game detail mobile surface (running example Elden Ring), as a live interactive Design Component in Crimson noir. **This ships the flagship verdict module in hi-fi**: immersive key-art hero with floating controls + tag strip; the three-tier verdict (Friends avatar stack · Completionists/your-type four-bucket sentiment distribution + neutral "strong match" relate callout · Global distribution + divergence insight), archetype badge, four-point grammar and crimson-restraint held; cost/time-to-beat; friend-activity rows; **trust-tier-tabbed reviews** (Friends / People like you / All) with animated swap; a "Played by Completionists" discovery carousel (image-slot covers, border-radius fix); a minimal non-blocking rate prompt; and a **sticky bottom bar → status sheet → Apple-Music-style "Add to a list" sheet** (live-filtering search, New list, tap-to-toggle add) with full reverse open/close animation. Also wired **Discover's spotlight to Elden Ring + its "View" → the detail page**. Tweakable props: gameTitle, initialStatus. Resolves three open threads (verdict-module hi-fi, mobile sheet interaction details, the "+ Add to a list" destination). Prior: second hi-fi screen — **`Library.dc.html`**, the Library / backlog mobile surface, as a live interactive Design Component in the Crimson noir dark theme. Games/Lists tabbed views; status-segmented backlog; collection cards with image-slot cover previews; a right-sliding **list detail** view with two layouts (ranked numbered rows + unordered vertical list) and per-list descriptions; two bottom sheets (time picker, lists) with full **reverse** open/close animation; and a full-screen **Edit List** screen (editable title + description, a ranked/unordered toggle that shows or hides position numbers, HTML5 drag-to-reorder, and an Add-games button). This resolves the previously-open custom-list-detail question. Prior: first hi-fi screen — **`Discover Home.dc.html`**, the Discover / Home mobile surface (spotlight hero with compressed two-rung why, four trust-ladder rails as carousels, at-a-glance cover cards with quick-add, cold-start Connect-Steam collapse, tab nav, image-slot covers, tweakable playerName/archetype/friendsConnected), which went to hi-fi ahead of the verdict module by choice to prove the system on a lower-risk surface first and surfaced a four-way vocabulary collision (Backlog / want list / list / Track). Before that: design-system foundations — crossed from lo-fi into visual design, foundations-first. Explored vibe directions, caught that the first pick ("Midnight editorial") was Claude's own palette, and pivoted to **Crimson noir** (wine-graphite `#241B1D` base, crimson `#E23E4E` accent, warm↔cool diverging sentiment, neutral-brightness trust ladder). Stress-tested it on the dense states and derived the **crimson restraint rule** (crimson = sentiment or primary action only). Locked type (**Familjen Grotesk + Newsreader**) and **ported a light theme** (cozy-paper cream `#F6F1E7`; trust ladder inverts, crimson deepens). All committed to `design-system-foundations.html`. Next milestone: verdict module to full hi-fi. Living document — supersedes handoff v2.*
